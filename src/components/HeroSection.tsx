...
import { Search, Loader2, Shield, Eye, AlertTriangle, Lock, Terminal, Copy, Check } from "lucide-react";
  ...
  return (
    <section className="relative flex-1 flex flex-col items-center justify-center px-4 py-8 sm:py-12">
      ...
      <motion.div
        ...
        className="flex items-center gap-3 px-4 py-2 rounded-full bg-critical/10 border border-critical/30 mb-8"
      >
        <AlertTriangle className="text-critical" size={16} />
      ...
      >
        <div className="relative group">
          <div className="absolute -inset-1 bg-gradient-to-r from-primary/30 via-critical/20 to-primary/30 rounded-2xl blur-lg opacity-50 group-hover:opacity-100 transition-opacity duration-500" />
          <div className="relative flex flex-col sm:flex-row gap-2 sm:gap-3 p-2 bg-card/90 backdrop-blur-xl rounded-2xl border border-border/50">
            <div className="flex-1 relative">
              ...
              <Input
                ...
                className="h-12 sm:h-14 pl-12 pr-4 text-sm sm:text-base bg-transparent border-0 focus:ring-0 focus-visible:ring-0 rounded-xl"
                disabled={isLoading}
              />
            </div>
            <Button
              ...
              size="lg"
              className="h-12 sm:h-14 px-6 sm:px-8 bg-gradient-to-r from-primary to-primary/80 text-primary-foreground hover:from-primary/90 hover:to-primary/70 rounded-xl font-bold text-sm sm:text-base shadow-lg shadow-primary/25"
            >
              {isLoading ? (
      ...
      >
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 bg-card/60 rounded-lg border border-success/30 hover:border-success/50 transition-colors">
          ...
          >
            {copied ? (
              ...
              <Copy size={12} className="text-muted-foreground hover:text-success" />
            )}
          </button>
        </div>
      </motion.div>
      ...
      >
        ...
        {EXAMPLE_WALLETS.map((wallet, index) => (
          <button
            ...
            disabled={isLoading}
            className="text-sm text-primary/80 hover:text-primary font-mono bg-card/50 px-3 py-1.5 rounded-lg border border-border/50 hover:border-primary/30 hover:bg-primary/5 transition-all disabled:opacity-50"
          >
      ...
      >
        ...
        <a
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-full bg-orange-500/10 border border-orange-500/30 hover:bg-orange-500/20 transition-colors"
        >
